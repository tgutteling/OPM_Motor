...
tmp=pickle.load(PCA_file)
PCA_saved['x'] = tmp
PCA_saved['y'] = tmp

#Initialize scaling
scaler={k : [] for k in wf_clean.keys()}
for k in wf_clean.keys():
    scaler[k] = RobustScaler().fit(wf_clean[k])    
...
PC_score_in_time = {k : {sub: { ep: 
                               ...
                               for k in PCA_saved.keys()}    
for i in PCA_saved.keys():
    for sub in metrics[i].keys():
        for ep in metrics[i][sub].keys():
            wvf = scaler[i].transform(metrics[i][sub][ep]["waveform"])
            metrics[i][sub][ep]["PC_score"] = PCA_saved[i].transform(wvf)
...
for  k in PCA_saved.keys():    
    for sub in metrics[k].keys():
        for ep in metrics[k][sub].keys():            
            for ft in imp_features:
                if ft == 'pp_ix':
                    data_frame_dict[k][ft].append(metrics[k][sub][ep][ft])
                else:                    
                    data_frame_dict[k][ft].extend(metrics[k][sub][ep][ft])            
            
            for pc_ix, pc_key in enumerate(pc_list):
                data_frame_dict[k][pc_key].extend(metrics[k][sub][ep]["PC_score"][:, pc_ix])
            
            waveform_array[k].append(metrics[k][sub][ep]["waveform"])
    
    waveform_array[k] = np.vstack(waveform_array[k])
burst_features= data_frame_dict

#component shape and timecourse
waveform_time = np.linspace(-0.13, 0.13, num=np.shape(waveform_array['x'])[1])
col_r = plt.cm.cool(np.linspace(0,1, num=10))

mean_waveform = {k : [] for k in waveform_array.keys()}
for k in waveform_array.keys():
    mean_waveform[k] = (np.mean(waveform_array[k], axis=0))
vis_time_plot = visual_time_bins[:-1]
mot_time_plot = motor_time_bins[:-1]

#Calculate specific burst rate
PC_to_analyse = ["PC_{}".format(pc_ix+1) for pc_ix in range(20)]
PC_burst_rate_spec={d: {i: {"vis": [], "mot": []} for i in PC_to_analyse} for d in burst_features.keys()}
subjects = np.unique(burst_features['x']['pp_ix'])
...
for pc_key in PC_to_analyse:
    ...
    for sens in burst_features.keys():
        comp_score = burst_features[sens][pc_key]
        ...
        for sub in subjects:
            sub_PC_br = {d: {
                "vis": [],
                "mot": []
            } for d in burst_features.keys()}
            for ep in ["vis", "mot"]:                
                peak_times=metrics[sens][sub][ep]['peak_time'][0]
                pc_scores=metrics[sens][sub][ep]['PC_score'][:,pc_ind]
                ...
                sub_PC_br[sens][ep] = PC_br
    
            for ep in ["vis", "mot"]:
                PC_burst_rate_spec[sens][pc_key][ep].append(sub_PC_br[sens][ep])
...
wvfrms = {x :
          {k: [] for k in pc_keys}
          for x in burst_features.keys()}

for x in burst_features.keys():    
    for pc_ix, pc_key in enumerate(pc_keys):
        for low, hi in prct_ranges:
            low_perc = np.percentile(burst_features[x][pc_key], low)
            hi_perc = np.percentile(burst_features[x][pc_key], hi)            
            wvf_ixs=np.where((burst_features[x][pc_key]>=low_perc) & (burst_features[x][pc_key]<=hi_perc))        
            MWF = np.mean(waveform_array[x][wvf_ixs, :], axis=1)
            wvfrms[x][pc_key].append(MWF)        
...
p=0.05
n_permutations=1000
for sens in sens_plot:
    
    #calculate significant for average burst rate
    sig_lines_vis_av=[]
    selection=np.array(group_metrics[sens]['vis']['burst_rate'])
    ...
    sig_lines_mot_av=[]
    selection=np.array(group_metrics[sens]['mot']['burst_rate'])
    ...
    for pc_ix, pc_key in enumerate(PC_to_analyse):  
        ...
        for pr in range(4):
            ax[pc_ix, 0].plot(waveform_time, wvfrms[sens][pc_key][pr].T*3e13 + np.mean([prct[pr], prct[pr+1]]), c=col_r[pr])
        ax[pc_ix, 0].plot(waveform_time, mean_waveform[sens]*3e13 + 50, c="black", lw=1, linestyle="dotted")
        
        # Time-burst rate
        selection_vis=np.mean(PC_burst_rate_spec[sens][pc_key]["vis"], axis=0).T
        selection_vis_all=PC_burst_rate_spec[sens][pc_key]["vis"].copy()
        selection_mot=np.mean(PC_burst_rate_spec[sens][pc_key]["mot"], axis=0).T
        selection_mot_all=PC_burst_rate_spec[sens][pc_key]["mot"].copy()
        # baselining
        bl_ix = np.where(
        ...
        divnorm = colors.TwoSlopeNorm(vmin=-max_abs_vis, vcenter=0, vmax=max_abs_vis)        
        ...
        divnorm = colors.TwoSlopeNorm(vmin=-max_abs_mot, vcenter=0, vmax=max_abs_mot)
       ...
       # BURSTxSCORE stat mot
        ...
        brate_mean_vis=group_metrics[sens]['vis']['burst_rate_av']
        brate_sem_vis=group_metrics[sens]['vis']['burst_rate_sem']
        ...
        bl_vis=np.zeros((len(prct_ranges),np.shape(PC_burst_rate_spec[sens][pc_key]["vis"])[0]))
        sig_lines_vis=[]
        for c_ix, (b, e) in enumerate(prct_ranges):
            ixes = np.where(((score_prc_range) >= b) & (score_prc_range <= e))[0]
            selection=np.array(PC_burst_rate_spec[sens][pc_key]["vis"])[:,:,ixes]
        ...
        brate_mean_mot=group_metrics[sens]['mot']['burst_rate_av']
        brate_sem_mot=group_metrics[sens]['mot']['burst_rate_sem']
        ...
        for c_ix, (b, e) in enumerate(prct_ranges):
            ixes = np.where(((score_prc_range) >= b) & (score_prc_range <= e))[0]
            selection=np.array(PC_burst_rate_spec[sens][pc_key]["mot"])[:,:,ixes]
            selection = np.nanmean(selection, axis=2)                    
        ...
        if len(sig_lines_vis)>0:
            nlines_vis=sig_lines_vis[-1][2]
        else:
            nlines_vis=0
        if len(sig_lines_mot)>0:
            nlines_mot=sig_lines_mot[-1][2]
        else:
            nlines_mot=0
        nlines=np.max([nlines_vis,nlines_mot])        
        ...
        for sl in sig_lines_vis:
            ax[pc_ix, 3].hlines(sig_y+((nlines-sl[2])*-.025),sl[0],sl[1],color=col_r[sl[2]])
        
        for sl in sig_lines_mot:
            ax[pc_ix, 4].hlines(sig_y+((nlines-sl[2])*-.025),sl[0],sl[1],color=col_r[sl[2]])
    ...
    ax[0, 1].set_xlabel("Time from Visual Stimulus onset [s]")    
    ax[0, 2].set_xlabel("Time from Motor response offset [s]")
    ax[0, 3].set_xlabel("Time from Visual Stimulus onset [s]")    
    ax[0, 4].set_xlabel("Time from Motor response offset [s]")
        
    plt.savefig(op.join(group_path,'Fig5_OPM_' +sens+ '_scaled.png'))
    plt.savefig(op.join(group_path,'Fig5_OPM_' +sens+ '_scaled.pdf'))